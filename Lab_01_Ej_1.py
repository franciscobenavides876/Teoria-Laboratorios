...
# ---------------------------------------------------------------------------
# 1. Definición de la clase DFA (Motor formal)
# ---------------------------------------------------------------------------
@dataclass
class DFA:
    states: Set[str]
...
# ---------------------------------------------------------------------------
# 2. Definición formal del Ejercicio 1 (Incorporación de SeleccionarMedico)
# ---------------------------------------------------------------------------
states_p1 = {
    "Inicio",
...
alphabet_p1 = {"especialidad", "medico", "fecha", "confirmar", "cancelar"}
...
dfa_p1 = DFA(states_p1, alphabet_p1, transition_p1, "Inicio", {"CitaAgendada"})


# ---------------------------------------------------------------------------
# 3. Simulación y visualización de resultados en consola
# ---------------------------------------------------------------------------
secuencia_p1 = ["especialidad", "medico", "fecha", "confirmar", "confirmar"]
estado_final_p1, log_p1 = dfa_p1.simulate(secuencia_p1)

print("\n" + "=" * 60)
print(" RESULTADOS DE EJECUCIÓN - EJERCICIO 1")
print("=" * 60)
print(f"Secuencia ingresada : {secuencia_p1}")
print(f"Estado final        : {estado_final_p1}")
print(f"¿Cita agendada?     : {estado_final_p1 in dfa_p1.accept_states}")

print("\n" + "-" * 60)
print(f"{'#':<3} | {'Estado actual':<24} | {'Entrada':<12} | {'Nuevo estado'}")
print("-" * 60)
for i, curr, sym, nxt in log_p1:
    print(f"{i:<3} | {curr:<24} | {sym:<12} | {nxt}")
print("-" * 60)


# ---------------------------------------------------------------------------
# 4. Generación y renderizado de la imagen del autómata (.PNG)
# ---------------------------------------------------------------------------
def guardar_grafo(dfa: DFA, log: List, final_state: str, filename="ejercicio_1"):
    ...
    try:
        ...
        print(f"\n[OK] Imagen del grafo generada exitosamente: {filename}.png")
    except Exception as e:
        print(f"\n[Aviso] Graphviz no pudo renderizar directamente: {e}")
        print("\nCódigo DOT generado (puedes pegarlo en dreampuf.github.io/GraphvizOnline):")
        print("-" * 40)
        print(dot.source)
        print("-" * 40)

guardar_grafo(dfa_p1, log_p1, estado_final_p1)